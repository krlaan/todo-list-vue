...
<template>
    <div class="container">
        <div class="header-row">
            <button class="create-button" @click="router.push('/CategoryCreate')">Create New
            </button>
        ...
        <div v-else class="sessions-list">
            <div v-for="item in data.data" :key="item.id" class="session-card">
                ...
                <div class="detail-row">
                    <span class="value">Sort: {{item.categorySort}}</span>
                </div>
                <div class="detail-row">
                    <span class="value">Tag: {{ item.tag }}</span>
                    <span class="links-container">
                    <RouterLink :to="`/CategoryDetails/${item.id}`" class="link">Details</RouterLink>
                        |
                    <RouterLink :to="`/CategoryUpdate/${item.id}`" class="link">Edit</RouterLink>
                        |
                    <RouterLink :to="`/CategoryDelete/${item.id}`" class="link">Delete</RouterLink>
                    </span>
                </div>
            </div>
        </div>
